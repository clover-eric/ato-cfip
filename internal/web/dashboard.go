...
<body>
  <div class="wrap">
    ...
    <div class="hero">
      <div>
        <div class="badge"><span class="dot" id="stateDot"></span><span id="stateText">载入中</span></div>
        <div class="domain">{{.Domain}}</div>
        <div class="sub">将业务、客户端或本地 DNS 指向这个域名，即可使用当前自动筛选出的优选 IP 池。</div>
      </div>
      <div class="stats">
    ...
    <div class="grid">
      ...
      <div class="side">
        ...
        <section>
          ...
          <div class="kv">
            <div class="row"><span>推荐发布</span><span>Cloudflare DNS only</span></div>
            <div class="row"><span>NAS 端口</span><span>8080</span></div>
            <div class="row"><span>无公网 IP</span><span>可用 Cloudflare Tunnel</span></div>
          </div>
        </section>
      </div>
    </div>
    <div class="foot">页面每 10 秒自动刷新。手动测速会在后台运行，期间会避免重复启动。</div>
  </div>
  ...
  <script>
    ...
    function renderTable(ips) {
      ...
      if (!ips.length) {
        ...
        wrap.textContent = '还没有发布结果。可以点击“立即测速”，或等待定时任务自动运行。';
        return;
      }
      wrap.className = '';
      wrap.innerHTML = '<table><thead><tr><th>#</th><th>IP</th><th>速度</th><th>延迟</th><th>丢包</th><th>地区</th><th>轮次</th></tr></thead><tbody>' +
